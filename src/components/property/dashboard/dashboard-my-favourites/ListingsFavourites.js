  ...
  const handleDeleteListing = async (id) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.showToast) {
        window.showToast('Property removed from favorites successfully!', 'success');
      }
    } catch (error) {
      console.error('Error removing favorite:', error);
      // Show error toast notification
      if (typeof window !== 'undefined' && window.showToast) {
        window.showToast('Failed to remove property from favorites', 'error');
      }
    } finally {
      setDeletingId(null);
    }
          ...
          <div className="col-md-6 col-lg-4 col-xl-3" key={listing.id}>
            <div className="listing-style1 style2">
              <div className="list-thumb">
                ...
                >
                  <span className={`fas fa-trash-can ${deletingId === listing.id ? 'text-muted' : ''}`}></span>
                </button>
